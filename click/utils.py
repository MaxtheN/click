...
def click_secret_key():
    _click = settings.PAYMENT_VARIANTS['click']
...
def click_webhook_errors(request):
    ...
    if is_set(request.POST,
              ['click_trans_id', 'service_id', 'click_paydoc_id', 'amount', 'action', 'error',
               'error_note', 'sign_time', 'sign_string']) or (
            action == '1' and is_set(request.POST, ['merchant_prepare_id'])):
        return {
            'error': '-8',
