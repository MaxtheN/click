...
class ApiHelper:
    ...
    def __init__(self, provider, payment, data, **kwargs):
        ...
        self.token = hashlib.sha1('{timestamps}{secret_key}'.format(
            timestamps=self.timestamps, secret_key=self.provider.secret_key
        ).encode('utf-8')
                                  ).hexdigest()
    ...
    def post(self, url, data):
        response = requests.post(self.endpoint + url, json=data, headers={
            ...
            'Auth': '{}:{}:{}'.format(self.provider.merchant_user_id, self.token, self.timestamps)
        })
        return response
    ...
    def payment_with_token(self):
        if self.payment.status != PaymentStatus.CONFIRMED:
            data = {
                ...
                "transaction_parameter": self.payment.transactions_id
            }
